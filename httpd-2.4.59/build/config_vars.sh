...
if test -f ${DESTDIR}/apache/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/apache/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/apache/bin/apu-1-config
else
...
test -n "/apache/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/apache/build"
